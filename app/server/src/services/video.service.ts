...
export async function saveVideoMetadata(
  ...
  const status = cfVideo?.status?.state === 'ready' ? 'ready' : 'processing'

  const playbackUrl = getPlaybackUrl(cloudflare_uid)
  const thumbnailUrl = metadata ? getThumbnailUrl(cloudflare_uid) : null
...
export async function deleteVideo(videoId: string, userId: string) {
  ...
  await deleteStreamVideo(video.cloudflare_uid)
